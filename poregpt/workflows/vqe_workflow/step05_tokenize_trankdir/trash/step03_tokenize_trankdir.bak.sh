...
INPUT_DIR="/mnt/nas_syy/default/poregpt/shared/dataset/dna/dna814g/trank" 

# Output directory for .jsonl.gz files
OUTPUT_DIR="/mnt/nas_syy/default/poregpt/shared/dataset/dna/dna814g/jsonlgz" 

# Path to your trained VQ tokenizer model checkpoint (.pth file)
MODEL_CHECKPOINT="/mnt/nas_syy/default/poregpt/poregpt/poregpt/workflows/vqe_workflow/step02_train_vqe_model/w64/models_scratch_cnn3/porepgt_vqe_tokenizer.step22500.pth"
...
for npy_file in "${all_files[@]}"; do
    ...
    if [ -f "$output_file" ]; then # Fixed: Removed space after =
        # echo "Skipping $output_file (already exists)" # Uncomment if you want to see skipped files
        ((skipped++))
        continue
    fi
    ...
    echo "➡️  Submitting $(basename "$npy_file") to GPU $gpu_id (output: $output_file)" >&2 
