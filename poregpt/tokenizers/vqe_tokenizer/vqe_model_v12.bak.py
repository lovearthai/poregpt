...
class NanoporeVQEModel_V12(nn.Module):
    """
    ...
    def decode_from_level_indices(self, level_indices: torch.Tensor) -> torch.Tensor:
        """
        使用 ResidualFSQ 原生方法从 level_indices 重建原始信号。

        Args:
            level_indices (torch.Tensor): [B, N, num_quantizers], dtype=torch.long
        """
        # 1. 确保 indices 是 Long 类型 (FSQ 需要此类型来查找码本)
        level_indices = level_indices.to(torch.long)
        B, N, n_q = level_indices.shape

        # 2. 调用 ResidualFSQ 的原生方法：
        # 它内部会执行：indices -> codes -> 乘以各层 scales -> 投影出 project_out (如果有)
        # 返回 z_quantized_projected: [B, N, fsq_level_n]
        z_quantized_projected = self.vq.get_output_from_indices(level_indices)

        # 3. 执行你模型自定义的 project_out (将 4 维映射回 CNN 的 128/512 维)
        # 注意：如果 self.vq 实例化时 dim == fsq_level_n，self.vq.project_out 是 Identity
        # 所以这里必须显式调用你模型层里的 self.project_out
        z_quantized_permuted = self.project_out(z_quantized_projected) # [B, N, C]

        # 4. 维度变换回 CNN 格式 [B, C, N]
        z_quantized = z_quantized_permuted.permute(0, 2, 1)
    ...
    def decode_from_first_layer(self, level_indices: torch.Tensor) -> torch.Tensor:
        """
        仅提取第一层量化信息重建信号。

        Args:
            level_indices (torch.Tensor): [B, N, n_q] 的完整索引
        """
        # 1. 只取第一层索引: [B, N, 1]
        first_layer_indices = level_indices[:, :, :1].to(torch.long)
        B, N, _ = first_layer_indices.shape

        # 2. 使用 ResidualFSQ 内置方法
        # 由于 n_q 变为了 1，get_output_from_indices 会自动只计算第一层
        # 且第一层的 scale = 1.0，不会有衰减
        z_q_first = self.vq.get_output_from_indices(first_layer_indices) # [B, N, fsq_level_n]

        # 3. 投影回 CNN 特征维度
        # 注意：即便只有一层量化，也要经过 project_out 映射回 C 维（如 512）
        z_q_permuted = self.project_out(z_q_first) # [B, N, C]
        z_quantized = z_q_permuted.permute(0, 2, 1) # [B, C, N]
        ...
        # 5. 长度对齐
        target_len = N * self.cnn_stride
        return F.pad(recon, (0, max(0, target_len - recon.shape[-1])))[:, :, :target_len]


    def tokenize_indices(self, level_indices: torch.Tensor) -> list[list[int]]:
        """
        将 level_indices 转换为唯一的 token ID 列表，
        完全复用 forward() 中 uni_indices 的计算逻辑。

        Args:
            level_indices (torch.Tensor): [B, N, K], K = self.num_quantizers

        Returns:
            List[List[int]]: 每个样本的 token ID 序列
        """
        if level_indices.dim() != 3:
            raise ValueError("level_indices must be a 3D tensor of shape [B, N, K]")

        B, N, K = level_indices.shape
        expected_K = self.num_quantizers  # 或 len(self.vq.levels)
        if K != expected_K:
            raise ValueError(f"Expected num_quantizers={expected_K}, but got {K}")

        device = level_indices.device

        # ✅ 关键：从 self 获取 fsq_levels，不依赖外部传参
        fsq_levels = self.vq.levels  # 这是一个 tuple 或 list，例如 (5, 5, 5, 4)
        fsq_size = math.prod(fsq_levels)

        # 构造权重：[fsq_size^(K-1), ..., fsq_size^0]
        exponents = torch.arange(K - 1, -1, -1, device=device, dtype=torch.long)
        fsq_size_tensor = torch.tensor(fsq_size, dtype=torch.long, device=device)
        multipliers = torch.pow(fsq_size_tensor, exponents)  # [K]

        # 加权求和
        weighted = level_indices.to(torch.long) * multipliers.view(1, 1, -1)
        uni_indices = torch.sum(weighted, dim=-1)  # [B, N]

        return uni_indices.cpu().tolist()
