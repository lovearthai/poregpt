...
class NanoporeVQEModel_V10(nn.Module):
    """
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        fsq_num_quantizers: int = 4,  # RFSQ 需要指定层数
        prevq_act: str ="none"
    ):
        """
        初始化 NanoporeVQModel。
        ...
        # 定义 FSQ 的量化级别
        # fsq_levels = [codebook_size, codebook_size, codebook_size, codebook_size] # 总码本大小为 16^4 = 65536
        # 3. 计算乘积: 4 * 4 * 4 * 5
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        # --- 在这里应用激活函数，然后送入 VQ ---
        #z_permuted_for_vq = self.activation_func(z_permuted)
        if self.prevq_act == "silu":
            z_permuted_for_vq = self.activation_func(z_permuted)
        elif self.prevq_act == "tanh":
            z_permuted_for_vq = torch.tanh(z_permuted)
        else:
            z_permuted_for_vq = z_permuted
        # 现象：CNN 输出的数值范围是不可控的。随着训练进行，权重可能会变大，导致输出从 [-1, 1] 变成 [-10, 10] 甚至 [-100, 100]。
        # 后果：
        # FSQ 的网格是有限的：FSQ 的量化公式通常是 round(x * scale)，它默认数据在 [-1, 1] 之间。
        # 溢出：如果输入是 5.0，而 FSQ 的最大量化值是 1.0，这个 5.0 就会被强制截断成 1.0。
        # 梯度消失：一旦数值被截断（Clamp），梯度就无法回传（梯度变为 0）。CNN 会发现自己无论怎么努力，输出都被“削顶”了，于是训练就会停滞或崩溃。
        # 加 tanh (软着陆)
        # 现象：tanh 函数会将任何实数（ −∞−∞ 到 +∞+∞ ）压缩到 (−1,1)(−1,1) 之间。
        # 好处：
        # 绝对安全：无论 CNN 输出多么离谱的数值，经过 tanh 后都会变成合法的、FSQ 能处理的数值。
        # 保护梯度：虽然 tanh 在两端梯度也会变小，但它是一个平滑的曲线，不会像硬截断那样直接“切断”梯度。


        # --- 应用结束 ---
        # 3. 应用 FSQ
        # z_quantized_permuted 形状维持 [B, N, C_CNN]
        # indices 形状为 [B, N]，是合并后的整型 Token ID
        
        # 3. 应用 Residual FSQ
        # quantized: 量化后的连续特征 [B, N, C_CNN]
        # level_indices: 每一层的索引 [B, N, num_quantizers]，例如 [4, 512, 4]
        z_quantized_permuted, level_indices = self.vq(z_permuted_for_vq)
        ...
        # 由于后续代码依赖这些返回值，你需要提供替代品。
        # all_loss: FSQ 没有像 VQ 那样的 commit loss 等，你可以返回一个零损失。
        all_loss = torch.tensor(0.0, device=z_permuted.device, requires_grad=False)

        # all_codes: FSQ 也没有这个概念，你可以返回 None 或一个空的 dict。
        all_codes = {} # 或者 None，取决于你后续代码如何处理它
        ...
        return recon, level_indices,  all_loss, uni_indices
