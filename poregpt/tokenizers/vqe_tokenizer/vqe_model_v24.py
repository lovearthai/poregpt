...
class NanoporeVQEModel_V24(nn.Module):
    """
    ...
    def __init__(
        ...
        self.transformer_encoder = LocalTransformerEncoder(
            ...
            window_size=65,
            dim_feedforward=D_transformer * 4, # 通常 FFN 是隐藏层的 4 倍
            dropout=dropout
        )

        # 第二阶段：128 -> 4
        self.project_in2 = nn.Linear(D_transformer, fsq_level_n)  # 128 -> 4
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        z_projected1 = self.project_in1(z_permuted) # [B, N, fsq_level_n]


        # 3. Transformer 增强 (在低维空间进行全局/局部建模) - 分块处理
        batch_size, seq_len, feat_dim = z_projected1.shape
        CHUNK_SIZE = 200
        CHUNK_OVERLAP = 65
        # 先添加位置编码，无论是否分块都需要
        z_low_dim = z_projected1 + self.pos_embedding[:, :seq_len, :]
        ...
        # 5. 第二阶段投影：128 -> 4
        # ======================================================================
        z_projected2 = self.project_in2(z_enhanced) # [B, N, 4]

        # 4. 应用 Residual FSQ
        # 注意这里传入的是 z_projected
        z_quantized_projected, level_indices = self.vq(z_projected2)
