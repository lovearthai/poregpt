...
class NanoporeVQEModel_V9(nn.Module):
    """
    ...
    def __init__(
        ...
        #fsq_levels = [4, 4, 4, 4] # 总码本大小为 16^4 = 65536
        print(fsq_levels)
        # 实例化 ResidualFSQ
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        # 3. 应用 Residual FSQ
        # quantized: 量化后的连续特征 [B, N, 128]
        # all_indices: 每一层的索引 [B, N, num_quantizers]
        z_quantized_permuted, indices = self.vq(z_permuted_for_vq)
        ...
        return recon, indices, all_loss, all_codes
