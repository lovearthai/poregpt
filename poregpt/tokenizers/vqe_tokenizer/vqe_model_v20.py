...
class NanoporeVQEModel_V20(nn.Module):
    """
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        z_projected_for_vq = self.project_in(z_permuted) # [B, N, fsq_level_n]

        
        z_vq, indices, all_loss, all_codes = self.vq(
            z_projected_for_vq, # 输入连续特征
