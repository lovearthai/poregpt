...
class NanoporeVQEModel_V23(nn.Module):
    """
    ...
    def __init__(
        ...
        # 1. 降维投影：CNN (128/256) -> FSQ 维度 (fsq_level_n)
        ...
        self.project_in = nn.Sequential(
            nn.Linear(cnn_output_dim, D_hidden),
            nn.SiLU(),           # 非线性激活
            nn.Linear(D_hidden, fsq_level_n)
        )
    
        # 2. Transformer Context Modeler (运行在低维空间)
        self.fsq_dim = fsq_level_n 
        self.max_position = 4096
        # 位置编码维度需匹配 fsq_level_n
        self.pos_embedding = nn.Parameter(torch.zeros(1, self.max_position, self.fsq_dim))
        nn.init.trunc_normal_(self.pos_embedding, std=0.01)

        # ⚠️ 注意：如果 fsq_level_n=4, nhead 可以是 1, 2, 4
        # 由于你在极低维度（如 4 维）运行 Transformer，nhead 不能设置得比维度还大。如果你的 fsq_level_n 是 4，而 nhead 也是 4，每个 head 只有 1 维特征，这可能导致模型表达能力受限。如果效果不好，建议增加 fsq_level_n 到 8 或 16，并相应调整 nhead。
        # 由于你的序列长度达到了 1200 个 token，在 4 维空间运行 Transformer 会面临“信息过载”的问题。以下是针对性建议：使用 Local Attention（局部注意力）：你代码中引用了 LocalTransformerEncoder。对于 1200 的长度，如果使用全局注意力，计算复杂度是 $1200^2$。使用局部窗口（如你代码中的 window_size=65）是明智的，因为它能让模型专注于 Nanopore 信号的局部波形特征。位置编码（Positional Embedding）的影响：在 4 维空间，位置编码占用的“能量”可能会盖过原始信号。建议：如果模型不收敛，尝试减小 pos_embedding 的初始化标准差（例如从 0.02 降到 0.01），或者干脆尝试不加位置编码（因为 CNN 本身带有平移不变性，对于信号处理有时足够了）
        nhead = 1
        num_layers = 4
        dropout = 0.1
        self.transformer_encoder = LocalTransformerEncoder(
            d_model=self.fsq_dim, # 这里必须是 fsq_level_n
            nhead=nhead,
            num_layers=num_layers,
            window_size=65,
            dim_feedforward=self.fsq_dim * 4, # 通常 FFN 是隐藏层的 4 倍
            dropout=dropout
        )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        # 3. Transformer 增强 (在低维空间进行全局/局部建模) - 分块处理
        batch_size, seq_len, feat_dim = z_projected.shape
        CHUNK_SIZE = 200
        CHUNK_OVERLAP = 65
        # 先添加位置编码，无论是否分块都需要
        z_low_dim = z_projected + self.pos_embedding[:, :seq_len, :]
        # 实际上，在分块处理时，我们仍然需要完整的全局位置编码，而不是每个块的局部位置编码。原因如下：
        # 为什么需要全局位置编码：
        # Transformer的全局感知能力：即使使用局部注意力，模型仍然需要知道每个token在整个序列中的绝对位置。
        # 跨块连接的一致性：重叠区域需要与前后块保持位置一致性。
        # 梯度传播的连续性：全局位置信息有助于梯度在不同块间正确传播。
        # 根据序列长度选择处理方式
        if seq_len > CHUNK_SIZE:
            # 使用分块处理以节省显存
            z_enhanced = self.transformer_encoder.forward_chunked(
                z_low_dim,
            ...
            # 短序列直接处理，效率更高
            z_enhanced = self.transformer_encoder(z_low_dim)

        # 4. 应用 Residual FSQ
        # 注意这里传入的是 z_projected
        z_quantized_projected, level_indices = self.vq(z_enhanced)
    ...
    def tokenize_indices(
        ...
        # 避免每次都计算
        local_codebook_size = self.codebook_size
