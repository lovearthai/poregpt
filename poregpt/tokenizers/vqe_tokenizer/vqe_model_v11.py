...
class NanoporeVQEModel_V11(nn.Module):
    """
    ...
    def __init__(
        ...
        assert self.cnn_model.out_channels == fsq_level_n, f"FSQ_LEVEL_N ({fsq_level_n}) must match number of cnn out channels ({self.cnn_model.out_channels})"
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        # ======================================================================
        # 输入 x: [B, 1, T]
        # 输出 z_cnn: [B, 4, N] (通道数=4, 长度 N=T//4)
        # 注意：由于 _build_cnn_type17 结尾是 nn.Tanh()，z_cnn 的值域已经被限制在 [-1, 1]
        z_cnn = self.cnn_model.encode(x)
        
        # ======================================================================
        # 2. 维度变换 (Permute)
        # ======================================================================
        # FSQ 期望输入格式: [B, Length, Channels]
        # CNN 输出格式:     [B, Channels, Length]
        ...
        # 输入: [B, N, 4]
        # 输出:
        #   z_quantized_permuted: [B, N, 4] (量化后的连续特征)
        #   level_indices:        [B, N, num_quantizers] (例如 [B, N, 2])
        z_quantized_permuted, level_indices = self.vq(z_permuted)
