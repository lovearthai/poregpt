...
def vqe_train(
    ...
    def evaluate_codebook_metrics_v2():
        ...
        token_counts = np.zeros(codebook_size, dtype=np.int64)
        total_tokens = 0
        total_eval_recon_loss = 0
        total_eval_comit_loss = 0
        num_batches = 0  # 添加批次计数器
        with torch.no_grad():
            ...
            for batch in pbar:
                ...
                recon, indices, _, loss_breakdown = model_for_eval(x)
                recon_loss = F.mse_loss(recon, x)
                comit_loss = loss_breakdown.commitment
                # 累积损失
                total_eval_recon_loss += recon_loss.item()  # 使用 .item() 获取标量值
                total_eval_comit_loss += comit_loss.item()  # 使用 .item() 获取标量值
        ...
        # 计算平均损失
        avg_eval_recon_loss = total_eval_recon_loss / num_batches if num_batches > 0 else 0
        avg_eval_comit_loss = total_eval_comit_loss / num_batches if num_batches > 0 else 0

        usage_ratio = len(used_codes) / codebook_size
        used_code_n = len(used_codes)
        top1_ratio = top3_ratio = top5_ratio = top7_ratio = top9_ratio = top10_ratio = 0.0
        entropy_val = 0.0
        max_entropy = np.log2(codebook_size)
        ...
        return (
            ...
            usage_ratio, total_tokens,
            top1_ratio, top3_ratio, top5_ratio, top7_ratio, top9_ratio, top10_ratio,
            entropy_val, max_entropy,avg_eval_recon_loss,avg_eval_comit_loss
        )
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if use_dynamic_commitment_weight and global_step > commitment_weight_freeze_steps:
                        expected_commitment_weight = g_recon_per_comit 
                        if dynamic_commitment_weight < expected_commitment_weight:
                            dynamic_commitment_weight += (expected_commitment_weight - dynamic_commitment_weight)*0.01
